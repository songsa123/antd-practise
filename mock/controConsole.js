...
function postList1(req, res, u) {
  ...
  return res.json(dataSource1);
}
function postAdd(req, res, u) {
  let url = u;
  if (!url || Object.prototype.toString.call(url) !== '[object String]') {
    url = req.url; // eslint-disable-line
  }
  const i = Math.ceil(Math.random() * 10000);
  let dataList = tableListDataSource.resultList.unshift({
    id: i,
    address: '',
    ip: '',
    prochamber: '',
  ...
  return getList1(req, res, u);
}
export default {
  'POST /api/controConsole': getList1,
  'POST /api/controConsoleSave/start1.action': postList1,
  'POST /api/add/add': postAdd,
};
